// ====== VÉDŐKERET – csak akkor induljon, ha a DOM kész és a PIXI is betöltődött ======
(function(){
  function showFatal(msg){
    const bar = document.createElement('div');
    bar.style.position='fixed'; bar.style.left='0'; bar.style.right='0'; bar.style.top='0';
    bar.style.padding='12px'; bar.style.zIndex='99999';
    bar.style.background='#ffdddd'; bar.style.color='#900';
    bar.style.fontFamily='monospace'; bar.style.boxShadow='0 2px 8px rgba(0,0,0,.2)';
    bar.textContent='Hiba: '+msg;
    document.body.appendChild(bar);
    console.error('[Animált Mese] '+msg);
  }

  function init(){
    try{
      if(!window.PIXI){ showFatal('A PixiJS nem töltődött be. Ellenőrizd a script URL-t és a "defer" sorrendet.'); return; }
      console.log('Animált Mese: init OK');
      ...
      function drawBackground(type='forest'){
        ...
        if (type==='space'){
          ...
          for(let i=0;i<140;i++){ const s=new PIXI.Graphics();
      ...
      document.getElementById('waveBtn').addEventListener('click', wave);
      ...
      // ====== Play Story (only checked animations) ======
      ...
      // ====== Auto story generation (1–5 perc) ======
      async function localGenerateStory(minutes){
        // nagyon egyszerű fallback generátor (ha nincs backend)
        const themes = ['erdei kaland','tengerparti nap','űrutazás','kisvárosi barátság','varázserdő'];
        ...
        for (let p=0;p<paras;p++){
          ...
          for (let s=0;s<sentences;s++){
            para.push(`${name} mosolyogva folytatta a kalandot, és minden fordulatban barátokra talált.`);
          }
          story += para.join(' ') + '\n\n';
        }
        story += 'A nap végén mindenki megpihent, és a kis hős megtanulta: a bátorság és a kedvesség mindig utat mutat. ';
        story += 'VÉGE.';
        return story;
      }
      ...
      autoBtn.addEventListener('click', async ()=>{
        ...
        // Ha van Netlify function (/.netlify/functions/chat), hívjuk meg,
        // ha nincs, essünk vissza helyi generálásra
        try{
          ...
          if(res.ok){
            ...
            if(data.story){ storyEl.value = data.story.trim(); return; }
          }
          // ha ide jutunk: nincs működő function -> fallback
